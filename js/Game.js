  ...
  checkForWin() {
    ...
    for (let i = 0; i < phrasesUl.children.length; i++) {
      if (phrasesUl.children[i].className.includes("hide")) {
        ...
        uniqueRemainingLetters.indexOf(newLetter) === -1
          ? uniqueRemainingLetters.push(newLetter)
    ...
    if (remainingLetters === 0) {
      ...
      sendMessageToBoard(true, "Youre almost there");
    }
  }
...
function sendMessageToBoard(isShown, message) {
  ...
  isShown
    ...
    : messageBoard.classList.remove("message-on");
}
